...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'OpenComputer Desktop 已就緒',
    desktopBootFailedWithMessage: message => `桌面啟動失敗：${message}`,
    steps: {
      connectingGateway: '正在連線桌面閘道',
      loadingSettings: '正在載入 OpenComputer 設定',
      loadingSessions: '正在載入最近工作階段',
      retryingRemoteBackend: '正在重新連線遠端 OpenComputer 後端…',
      startingDesktopConnection: '正在啟動桌面連線',
      startingHermesDesktop: '正在啟動 OpenComputer Desktop…'
    },
    errors: {
      backgroundExited: 'OpenComputer 背景程序已結束。',
      backgroundExitedDuringStartup: 'OpenComputer 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 OpenComputer 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 OpenComputer Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒絕',
      inputTitle: '需要輸入',
      inputBody: 'OpenComputer 正在等待你的回應。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 OpenComputer 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 OpenComputer 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'OpenComputer 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'OpenComputer 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '背景回合錯誤。'
        ...
        plugin: {
          label: '外掛通知',
          description: 'OpenComputer 在背景時，桌面外掛傳送了通知。'
        }
      },
      test: '傳送測試通知',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 OpenComputer 跟隨系統設定。',
      ...
      reactionsTitle: '訊息回應',
<<<<<<< HEAD
      reactionsDesc: 'iMessage 風格的表情回應 — 你可以對訊息做出回應，OpenComputer 也能回應你的訊息。',
=======
...
>>>>>>> upstream/main
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 OpenComputer 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 OpenComputer，然後回到此處。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'OpenComputer 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'OpenComputer 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'OpenComputer 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'OpenComputer 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'OpenComputer 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'OpenComputer 執行環境已更新,但桌面應用程式本身仍是舊建置——在應用程式更新之前,新的介面功能(如 Bot Mode)不會顯示。請執行下方的更新以重新建置應用程式。如果此警告仍未消除,請從最新的桌面安裝程式重新安裝。',
      ...
      automaticUpdatesDesc: 'OpenComputer 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速輸入',
      enabledDesc: '用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 OpenComputer 即可送出提示。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'OpenComputer Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 OpenComputer 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 OpenComputer 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
        '將此桌面殼層連線至遠端 OpenComputer 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'OpenComputer Desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · OpenComputer ${version}` : ''}`,
      ...
      sshTitle: '透過 SSH 連線',
      sshDesc:
        'OpenComputer 會透過 SSH 在遠端啟動並以通道連線到本應用程式——無需自行啟動或公開任何服務。前提：已具備到該主機的金鑰 SSH 存取。',
      ...
      sshHermesPathTitle: 'OpenComputer 路徑（選用）',
      ...
      sshReachable: (host, platform) => `可連線：${host}（${platform}）——已找到 OpenComputer`,
      sshIncompleteHost: '連線前請輸入 SSH 主機。',
      sshErrUnreachable: '無法透過 SSH 連線到該主機。請檢查主機、連接埠和網路。',
      sshErrAuth:
        'SSH 驗證失敗。請將金鑰載入 ssh-agent（ssh-add），或在 ~/.ssh/config 中設定 IdentityFile——OpenComputer 以非互動方式執行 ssh。',
      sshErrHostKey: '自上次連線以來主機金鑰已變更。請確認這是預期的，然後執行 ssh-keygen -R <host> 並重新連線。',
      sshErrNotInstalled:
        '遠端主機上未安裝 OpenComputer。請在遠端安裝（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）或設定 OpenComputer 路徑。',
      sshErrPlatform: '不支援的遠端平台。OpenComputer Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
      sshErrTimeout: 'SSH 連線逾時。主機可能無法存取或處於睡眠狀態。',
      sshErrUpdateRequired: '使用 Desktop SSH 連線前，請更新遠端主機上的 OpenComputer。',
    ...
    mcp: {
      ...
      deepLinkDescription: '一個連結要求將此 MCP 伺服器加入 OpenComputer。請檢查下方的完整設定——它來自該連結，而非 OpenComputer。',
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。OpenComputer 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
      ...
      localEndpoint: {
        title: '本地 / 自訂端點',
        description: '將 OpenComputer 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在載入提供方...'
    },
    sessions: {
  ...
  starmap: {
    ...
    loading: '載入中…',
    emptyTitle: '尚無學習內容',
    emptyDesc: '當 OpenComputer 為你的工作建立技能與記憶時，會顯示在這裡。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 OpenComputer 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 OpenComputer 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 OpenComputer 會通知你。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 OpenComputer 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `OpenComputer ${version} · 活躍工作階段 ${count}`,
  ...
  profiles: {
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 OpenComputer 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      review: '檢查排程工作',
<<<<<<< HEAD
      saveFailed: 'OpenComputer 未儲存該模型變更。'
=======
...
>>>>>>> upstream/main
    ...
    newCron: '新排程工作',
    emptyDescNew: '按 cron 表達式排程一個提示詞。OpenComputer 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      staleBackend: '請更新 OpenComputer 後端以建立專案——目前後端比桌面應用舊（設定 → 更新 → 後端）。',
      deleteConfirm: '這會從 OpenComputer 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。',
      startWork: '新增工作樹',
      ...
      worktreeStaleBackend: '請更新 OpenComputer 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 OpenComputer...',
    placeholderReconnecting: '正在重新連線至 OpenComputer…',
    ...
    attachUrlDesc: 'OpenComputer 將擷取該頁面並作為此回合的脈絡。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: '讓 OpenComputer 提交並開 PR',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 OpenComputer 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 OpenComputer 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 OpenComputer 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 OpenComputer，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 OpenComputer 時會使用新版本。',
    guiSkewTitle: '請更新桌面應用程式',
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 OpenComputer 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
      'OpenComputer 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 OpenComputer。更新期間請勿自行重新開啟 OpenComputer。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 OpenComputer 會自動重新連線。',
    applyingClose: '此視窗會在更新期間關閉，隨後 OpenComputer 會自動重新開啟。',
    errorTitle: '更新未完成',
    errorBody: '沒有資料遺失。您可以現在重試。',
    blockerTitle: '關閉本機預覽以更新 OpenComputer？',
    blockerBody: 'OpenComputer 需要在更新前停止這些本機預覽。這不會修改或刪除您的檔案。',
    foreignBlockerTitle: '關閉其他處理程序以更新 OpenComputer',
    foreignBlockerBody:
      'OpenComputer 無法安全地自動關閉這些處理程序。請關閉擁有這些處理程序的應用程式、終端機或服務，然後重試更新。',
    mixedBlockerBody: 'OpenComputer 可以關閉下方列出的本機預覽。其他處理程序必須手動關閉，更新才能繼續。',
  ...
  install: {
    ...
    oneTimeTitle: 'OpenComputer 需要一次性安裝',
    ...
    setupChoiceTitle: '設定 OpenComputer Desktop',
    setupChoiceDesc: '將此應用程式連線到您已執行的 OpenComputer 閘道，或在這台電腦上本機安裝 OpenComputer。',
    connectExistingTitle: '連線到現有 OpenComputer',
    connectExistingShort: '連線現有環境',
    connectExistingDesc: '使用工作階段權杖或瀏覽器登入連線遠端後端。不會啟動本機安裝。',
    installLocalTitle: '本機安裝 OpenComputer',
    installLocalDesc: '下載 OpenComputer、建立 Python 環境，並在這台電腦上執行後端。',
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 OpenComputer Desktop 後再試一次。',
    remoteSetupTitle: '連線到現有 OpenComputer',
    remoteSetupDesc: '輸入閘道 URL。OpenComputer Desktop 會偵測需要權杖還是瀏覽器登入。',
    remoteUrlTitle: '閘道 URL',
    remoteUrlDesc: '使用 OpenComputer 閘道的基礎 URL；遠端位址請包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在偵測閘道驗證方式...',
    probeError: '無法連線到該 OpenComputer 閘道。',
    ...
    backToSetup: '返回',
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 OpenComputer Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 OpenComputer CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 OpenComputer 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。OpenComputer 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 OpenComputer Agent',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'OpenComputer 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 OpenComputer…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 OpenComputer 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 OpenComputer 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — OpenComputer 會自動連線',
    ...
    authorizeThere: '請在那裡授權 OpenComputer。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 OpenComputer，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。OpenComputer 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'OpenComputer 正在重新啟動...',
      askRestart: '請 OpenComputer 重新啟動伺服器',
      lookingRestart: taskId => `OpenComputer 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'OpenComputer 正在背景執行。可在預覽主控台查看進度。',
      ...
      blankPageBody: '在上方輸入網址開始瀏覽，或請 OpenComputer 開啟頁面。',
      finishedRestarting: message => `OpenComputer 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'OpenComputer 無法重新啟動伺服器。',
      stillWorking: 'OpenComputer 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '顯示較早的訊息',
      loadingResponse: 'OpenComputer 正在載入回覆',
    ...
    approval: {
      gatewayDisconnected: 'OpenComputer 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      command: '指令',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.hermes/config.yaml）。OpenComputer 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'OpenComputer 需要您的 sudo 密碼來執行特權指令。它只會傳送給您的本機代理。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'OpenComputer 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 OpenComputer Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 OpenComputer Desktop 以儲存圖片',
