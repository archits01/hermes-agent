...
export function useModelControls({ queryClient, requestGateway }: ModelControlsOptions) {
  ...
  const selectModel = useCallback(
    async (selection: ModelSelection): Promise<boolean> => {
      ...
      try {
        // The composer picker changes THIS conversation only. Persisting a
        // primary-session click via --global made casual model trials rewrite
        // model.default/model.provider for the whole profile and every future
