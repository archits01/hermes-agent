...
export const pt: Translations = {
  ...
  status: {
    ...
    stopped: "Parado",
    updateHermes: "Atualizar OpenComputer",
    updatingHermes: "A atualizar OpenComputer…",
  ...
  pluginsPage: {
    ...
    headline:
      "Descobrir, instalar, ativar e atualizar plugins OpenComputer (paridade com `hermes plugins`).",
  ...
  skills: {
    wisdom: {
      notificationPreferences: {
        ...
        scope: "As suas notificações proativas nos clientes desta organização. A consulta e a partilha manuais continuam disponíveis.",
        ...
        pending: "A sua escolha foi guardada localmente e aguarda sincronização.",
        failed: "A sincronização falhou. Atualize as definições antes de escolher novamente.",
        conflict: "A sua preferência mudou noutro cliente. Atualize para a rever.",
        expired: "Esta escolha expirou. Atualize as definições para tentar novamente.",
      },
      tab: 'Coletivo',
      browseHub: 'Explorar o Hub',
      title: 'Sabedoria coletiva',
      loading: 'A carregar a Sabedoria coletiva',
      unavailable: 'A Sabedoria coletiva não está disponível.',
      setup: 'A Sabedoria coletiva não está configurada para este perfil.',
      setupDisclosure:
        'A qualificação de candidatos permanece neste perfil. Apenas o conteúdo do rascunho privado aprovado pelo proprietário, o texto do autor, os metadados declarativos do manifesto e o estado das instalações geridas chegam ao Gateway.',
      setupAction: 'Compreendo — configurar este perfil',
      settingUp: 'A configurar…',
      scanLocal: 'Analisar competências locais',
      search: 'Pesquisar competências partilhadas…',
      potential: 'Contribuições sugeridas',
      potentialHelp:
        'Competências qualificadas localmente pelo Hermes com base no uso ou em melhorias significativas. Nada é partilhado antes da tua revisão.',
      noSuggestions: 'Atualmente, nenhuma competência local cumpre as regras de qualificação automática.',
      browseLocal: count => `Ver todas as competências locais (${count})`,
      browseLocalHelp:
        'A seleção manual não significa que o Hermes tenha usado ou qualificado automaticamente estas competências.',
      ownerReview: 'Os teus rascunhos de contribuição',
      ownerReviewHelp: 'Rascunhos à espera da tua revisão e submissões à espera da aprovação do coletivo.',
      noDrafts: 'Não há rascunhos ou submissões de contribuição ativos.',
      prepare: 'Iniciar contribuição',
      continueDraft: 'Continuar rascunho',
      localOnly: 'Disponível para partilhar a partir deste dispositivo',
      qualifiedLocally: 'O Hermes identificou esta competência local como uma possível contribuição.',
      qualificationFirst: organizationName =>
        `${organizationName ? `A sua organização (${organizationName})` : 'A sua organização'} ativou o Collective Wisdom, uma funcionalidade concebida para detetar automaticamente competências úteis entre todos os membros da equipa. Parabéns! O Hermes detetou uma competência que poderá ser útil para a sua equipa.`,
      qualificationReturning: 'O Hermes detetou outra competência que poderá ser útil para a sua equipa.',
      savedLocally: 'Foi guardado um rascunho privado neste dispositivo.',
      reviewExact: 'Ver detalhes',
      draftState: state => {
        const labels: Record<string, string> = {
          vetting: 'Revisão do servidor em curso',
          ready: 'Pronto para a tua revisão',
          owner_approved: 'Aprovado por ti',
          publishing: 'A publicar',
          pending_moderation: 'A aguardar aprovação do administrador do coletivo',
          changes_requested: 'Alterações solicitadas'
        }
        return labels[state] || state.replaceAll('_', ' ')
      },
      serverScanPassed: 'análise do servidor aprovada',
      noDescription: 'Sem descrição escrita pelo proprietário.',
      managedInstalls: 'instalações geridas',
      close: 'Fechar',
      readEvery: 'Lê cada ficheiro em bruto. A aprovação está vinculada aos três hashes exatos apresentados abaixo.',
      editReview:
        'Edita aqui a descrição, o SKILL.md ou o manifesto declarativo. Guardar cria uma nova revisão privada, repete as análises e devolve novos hashes; não reescreve a competência de origem local.',
      unsavedChanges: 'Estas alterações ainda não foram analisadas. Guarda e volta a analisar antes de aprovar.',
      saveAndRescan: 'Guardar alterações e voltar a analisar',
      savingRevision: 'A guardar e a analisar novamente…',
      resetChanges: 'Descartar alterações',
      reviewedHashes: 'Hashes da revisão atualmente analisada pelo servidor',
      prepareTitle: 'Rever o pacote local antes de carregar',
      prepareNotice:
        'Estes campos permanecem neste perfil até serem explicitamente submetidos a uma revisão privada do servidor para o proprietário. As contagens e os motivos de qualificação locais nunca são incluídos.',
      ownerDescription: 'Descrição escrita pelo proprietário',
      systemSpecification: 'Especificação do sistema (metadados declarativos; nenhuma dependência é instalada)',
      localOverlay: 'Sobreposição local',
      cancel: 'Cancelar',
      submit: 'Submeter rascunho',
      submitting: 'A submeter…',
      publishing: 'A publicar…',
      approve: 'Aprovar o conteúdo exato e publicar',
      decline: 'Recusar',
      checkUpdates: count => `Procurar atualizações${count ? ` (${count})` : ''}`,
      checking: 'A verificar…',
      refreshShared: 'Atualizar skills partilhadas',
      refreshingShared: 'A atualizar…',
      installReferenceLabel: 'Instalar a partir de uma ligação ou ID da skill',
      installReferencePlaceholder: 'Cola uma ligação do Portal, um ID da skill ou skill-id@vN',
      installReferenceHelp:
        'O Hermes verifica a versão exata e mostra um plano de compatibilidade antes da instalação.',
      reviewInstall: 'Rever instalação',
      planningInstall: 'A validar…',
      updateModeLabel: 'Atualizações futuras',
      updateModeDefault: 'Usar a predefinição da organização',
      updateModeManual: 'Manual',
      updateModeAutomatic: 'Automática com aviso',
      updateModeRequired: 'Obrigatória',
      updateModeHelp:
        'O Gateway aplica a política atual da sua organização. Alterações sensíveis à segurança continuam a exigir a sua aprovação.',
      updateModePlan: mode => `Atualizações futuras: ${mode}`,
      activityReady: count => `${count} ${count === 1 ? 'nova notificação' : 'novas notificações'}`,
      decisionPublished: skill => `${skill} foi aprovada e está agora partilhada com a tua equipa.`,
      decisionChanges: skill => `${skill} precisa de alterações antes de poder ser partilhada.`,
      ...
      takedownNotice: skill => `${skill} foi removida do coletivo.`,
      markSeen: 'Marcar tudo como lido',
      install: 'Instalar…',
      uninstall: 'Desinstalar…',
      checkSkill: 'Verificar esta competência',
      updateAvailable: version => (version ? `Atualização v${version} disponível` : 'Atualização disponível'),
      reviewUpdate: 'Rever atualização',
      installed: (version, mode) => `Instalada v${version} · ${mode}`,
      confirmAction: action => `Confirmar ${action}`,
      acceptCompatibility: 'Revi e aceito as ações de compatibilidade.',
      acceptSensitive: 'Aceito explicitamente os novos requisitos sensíveis.',
      preserveModified: 'Preservar primeiro a minha cópia modificada como uma bifurcação não gerida.'
    },
    title: "Competências",
  ...
  env: {
    ...
    customHint: "Variáveis de ambiente arbitrárias armazenadas no seu .env que o OpenComputer não reconhece. Use-as para injetar variáveis de ambiente para skills, servidores MCP ou suas próprias ferramentas.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Distintivos colecionáveis do OpenComputer obtidos a partir do histórico real de sessões. Conquistas conhecidas mas ainda não obtidas aparecem como Descobertas; conquistas Secretas permanecem ocultas até surgir o primeiro comportamento correspondente.",
      scan_subtitle:
        "A analisar o histórico de sessões do OpenComputer. A primeira análise pode demorar 5–10 segundos em históricos extensos.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Mais recente",
      latest_hint_empty: "execute mais o OpenComputer",
    ...
    guide: {
      ...
      secret_body:
        "As secretas escondem o seu acionador exato. Assim que o OpenComputer detetar um sinal relacionado, o cartão passa a Descoberta e mostra o requisito.",
      scan_status_header: "Estado da análise",
      scan_status_body:
        "O OpenComputer analisa o histórico local uma vez e depois os cartões aparecem automaticamente. Nada está bloqueado se isto demorar alguns segundos.",
