...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateHermes: "Обновить OpenComputer",
    updatingHermes: "Обновление OpenComputer…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов OpenComputer (аналог `hermes plugins`).",
  ...
  skills: {
    wisdom: {
      ...
      tab: 'Коллектив',
      browseHub: 'Открыть Hub',
      title: 'Коллективные знания',
      loading: 'Загрузка Коллективных знаний',
      unavailable: 'Коллективные знания недоступны.',
      setup: 'Коллективные знания не настроены для этого профиля.',
      setupDisclosure:
        'Квалификация кандидатов остаётся в этом профиле. В Gateway передаются только одобренные владельцем данные приватного черновика, авторский текст, декларативные метаданные манифеста и состояние управляемой установки.',
      setupAction: 'Понятно — настроить этот профиль',
      settingUp: 'Настройка…',
      scanLocal: 'Сканировать локальные навыки',
      search: 'Искать общие навыки…',
      potential: 'Предлагаемые вклады',
      potentialHelp:
        'Навыки, локально квалифицированные Hermes на основе использования или значимых улучшений. До вашей проверки ничего не публикуется.',
      noSuggestions: 'Сейчас ни один локальный навык не соответствует правилам автоматической квалификации.',
      browseLocal: count => `Показать все локальные навыки (${count})`,
      browseLocalHelp: 'Ручной выбор не означает, что Hermes использовал или автоматически квалифицировал эти навыки.',
      ownerReview: 'Ваши черновики вкладов',
      ownerReviewHelp: 'Черновики, ожидающие вашей проверки, и заявки, ожидающие одобрения коллектива.',
      noDrafts: 'Нет активных черновиков или заявок на вклад.',
      prepare: 'Начать вклад',
      ...
      draftState: state => {
        const labels: Record<string, string> = {
          vetting: 'Идёт проверка сервером',
          ready: 'Готово к вашей проверке',
          owner_approved: 'Одобрено вами',
          publishing: 'Публикация',
          pending_moderation: 'Ожидается одобрение администратора коллектива',
          changes_requested: 'Запрошены изменения'
        }
        return labels[state] || state.replaceAll('_', ' ')
      },
      serverScanPassed: 'проверка сервером пройдена',
      noDescription: 'Нет описания от владельца.',
      managedInstalls: 'управляемых установок',
      close: 'Закрыть',
      readEvery: 'Прочитайте каждый исходный файл. Одобрение привязано к трём точным хешам ниже.',
      editReview:
        'Здесь можно изменить описание, SKILL.md или декларативный манифест. Сохранение создаёт новую приватную редакцию, повторно запускает проверки и возвращает новые хеши; локальный исходный навык не перезаписывается.',
      unsavedChanges: 'Эти изменения ещё не проверены. Сохраните и повторно просканируйте их перед одобрением.',
      saveAndRescan: 'Сохранить и повторно сканировать',
      savingRevision: 'Сохранение и повторное сканирование…',
      resetChanges: 'Отменить изменения',
      reviewedHashes: 'Хеши редакции, проверенной сервером',
      prepareTitle: 'Проверьте локальный пакет перед загрузкой',
      prepareNotice:
        'Эти поля остаются в профиле, пока вы явно не отправите их на приватную серверную проверку владельца. Локальные показатели и причины квалификации никогда не включаются.',
      ownerDescription: 'Описание от владельца',
      ...
      decline: 'Отклонить',
      checkUpdates: count => `Проверить обновления${count ? ` (${count})` : ''}`,
      checking: 'Проверка…',
      refreshShared: 'Обновить общие навыки',
      refreshingShared: 'Обновление…',
      installReferenceLabel: 'Установить по ссылке или ID навыка',
      installReferencePlaceholder: 'Вставьте ссылку Portal, ID навыка или skill-id@vN',
      installReferenceHelp: 'Hermes проверит точную версию и покажет план совместимости перед установкой.',
      reviewInstall: 'Проверить установку',
      planningInstall: 'Проверка…',
      updateModeLabel: 'Будущие обновления',
      updateModeDefault: 'Использовать настройку организации',
      updateModeManual: 'Вручную',
      updateModeAutomatic: 'Автоматически с уведомлением',
      updateModeRequired: 'Обязательно',
      updateModeHelp:
        'Gateway применяет текущую политику организации. Изменения, затрагивающие безопасность, по-прежнему требуют вашего одобрения.',
      updateModePlan: mode => `Будущие обновления: ${mode}`,
      activityReady: count => `${count} новых уведомлений`,
      decisionPublished: skill => `${skill} одобрен и теперь доступен вашей команде.`,
      decisionChanges: skill => `${skill} нужно изменить перед публикацией.`,
      ...
      takedownNotice: skill => `${skill} удалён из коллектива.`,
      markSeen: 'Отметить всё прочитанным',
      install: 'Установить…',
      uninstall: 'Удалить…',
      checkSkill: 'Проверить этот навык',
      updateAvailable: version => (version ? `Доступно обновление v${version}` : 'Доступно обновление'),
      reviewUpdate: 'Просмотреть обновление',
      installed: (version, mode) => `Установлено v${version} · ${mode}`,
      confirmAction: action => `Подтвердить: ${action}`,
      acceptCompatibility: 'Я проверил(-а) и принимаю действия по совместимости.',
  ...
  env: {
    ...
    customHint: "Произвольные переменные окружения, сохранённые в вашем .env, которые OpenComputer не распознаёт. Используйте их для внедрения переменных окружения для навыков, серверов MCP или собственных инструментов.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки OpenComputer, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий OpenComputer. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте OpenComputer чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только OpenComputer обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "OpenComputer сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
