...
export const zh: Translations = {
  ...
  status: {
    ...
    updatingHermes: "正在更新 OpenComputer…",
  ...
  pluginsPage: {
    ...
    headline: "发现、安装、启用和更新 OpenComputer 插件（对齐 `hermes plugins` CLI）。",
  ...
  skills: {
    wisdom: {
      ...
      managedInstalls: '个托管安装',
      close: '关闭',
      readEvery: '请阅读下方每个原始文件。批准将绑定到显示的三个准确哈希值。',
      editReview:
      ...
      prepareNotice:
        '在你明确提交到仅所有者可见的服务器审核前，这些字段会保留在此配置文件中。本地资格计数和原因永远不会包含在内。',
  ...
  env: {
    ...
    hideValue: "隐藏值",
    customTitle: "自定义密钥",
    customHint: "存储在 .env 中、OpenComputer 无法识别的任意环境变量。可用于为技能、MCP 服务器或你自己的工具注入环境变量。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "从真实会话历史中获得的 OpenComputer 可收集徽章。已知尚未达成的成就显示为「已发现」；秘密成就在首次出现匹配行为之前保持隐藏。",
      scan_subtitle:
        "正在扫描 OpenComputer 会话历史。在历史记录较多时，首次扫描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新扫描",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "多多运行 OpenComputer",
      none_yet: "暂无",
    },
    state: {
      unlocked: "已解锁",
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就会隐藏其确切触发条件。一旦 OpenComputer 检测到相关信号，卡片将变为「已发现」并显示其要求。",
      scan_status_header: "扫描状态",
      scan_status_body:
        "OpenComputer 正在对本地历史进行一次扫描，之后卡片会自动出现。即使这需要几秒钟，也没有卡住。",
