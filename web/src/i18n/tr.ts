...
export const tr: Translations = {
  ...
  skills: {
    wisdom: {
      ...
      loading: 'Kolektif Bilgelik yükleniyor',
      unavailable: 'Kolektif Bilgelik kullanılamıyor.',
      setup: 'Kolektif Bilgelik bu profil için ayarlanmamış.',
      setupDisclosure:
        'Aday yeterlilik bilgileri bu profilde kalır. Gateway’e yalnızca sahibinin onayladığı özel taslak içeriği, yazar metni, bildirimsel manifest meta verileri ve yönetilen kurulum durumu ulaşır.',
      setupAction: 'Anladım — bu profili ayarla',
      settingUp: 'Ayarlanıyor…',
      scanLocal: 'Yerel becerileri tara',
      ...
      draftState: state => {
        const labels: Record<string, string> = {
          vetting: 'Sunucu incelemesi sürüyor',
          ready: 'İncelemeniz için hazır',
          owner_approved: 'Tarafınızdan onaylandı',
          publishing: 'Yayımlanıyor',
          pending_moderation: 'Kolektif yöneticisinin onayı bekleniyor',
          changes_requested: 'Değişiklik istendi'
        }
        return labels[state] || state.replaceAll('_', ' ')
      },
      serverScanPassed: 'sunucu taraması geçti',
      noDescription: 'Sahibi tarafından yazılmış açıklama yok.',
      managedInstalls: 'yönetilen kurulum',
      close: 'Kapat',
      readEvery: 'Her ham dosyayı okuyun. Onay, aşağıda gösterilen üç kesin karmaya bağlıdır.',
      editReview:
        'Açıklamayı, SKILL.md dosyasını veya bildirimsel manifesti burada düzenleyin. Kaydetmek yeni bir özel revizyon oluşturur, taramaları yeniden çalıştırır ve yeni karmalar döndürür; yerel kaynak becerinizi yeniden yazmaz.',
      unsavedChanges: 'Bu değişiklikler taranmadı. Onaylamadan önce kaydedip yeniden tarayın.',
      saveAndRescan: 'Değişiklikleri kaydet ve yeniden tara',
      savingRevision: 'Kaydediliyor ve yeniden taranıyor…',
      resetChanges: 'Düzenlemeleri at',
      reviewedHashes: 'Sunucunun şu anda incelediği revizyonun karmaları',
      prepareTitle: 'Yüklemeden önce yerel paketi inceleyin',
      prepareNotice:
        'Bu alanlar, yalnızca sahibin görebildiği sunucu incelemesine açıkça gönderene kadar bu profilde kalır. Yerel yeterlilik sayıları ve nedenleri hiçbir zaman eklenmez.',
      ownerDescription: 'Sahibi tarafından yazılmış açıklama',
      ...
      decline: 'Reddet',
      checkUpdates: count => `Güncellemeleri denetle${count ? ` (${count})` : ''}`,
      checking: 'Denetleniyor…',
      refreshShared: 'Paylaşılan becerileri yenile',
      ...
      updateModeDefault: 'Kuruluş varsayılanını kullan',
      updateModeManual: 'Manuel',
      updateModeAutomatic: 'Bildirimli otomatik',
      updateModeRequired: 'Zorunlu',
      updateModeHelp:
        'Gateway kuruluşunuzun güncel politikasını uygular. Güvenlik açısından hassas değişiklikler yine onayınızı gerektirir.',
      updateModePlan: mode => `Gelecekteki güncellemeler: ${mode}`,
      activityReady: count => `${count} yeni bildirim`,
      decisionPublished: skill => `${skill} onaylandı ve artık ekibinizle paylaşılıyor.`,
      decisionChanges: skill => `${skill} paylaşılmadan önce değişiklik gerektiriyor.`,
      ...
      takedownNotice: skill => `${skill} kolektiften kaldırıldı.`,
      markSeen: 'Tümünü okundu işaretle',
      install: 'Kur…',
      uninstall: 'Kaldır…',
      checkSkill: 'Bu beceriyi denetle',
      updateAvailable: version => (version ? `v${version} güncellemesi hazır` : 'Güncelleme hazır'),
      reviewUpdate: 'Güncellemeyi incele',
      installed: (version, mode) => `Kurulu v${version} · ${mode}`,
      confirmAction: action => `${action} işlemini onayla`,
      acceptCompatibility: 'Uyumluluk işlemlerini inceledim ve kabul ediyorum.',
