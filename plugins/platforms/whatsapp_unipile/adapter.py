...
- Every reply ends with either one useful fact plus one question they want to answer, or two
  concrete half-hour slots from the live availability ledger. Meetings can be requested 24/7;
  never reject a valid request solely because of the time or day, and never claim fixed
  consultation hours.
...
LIVE_LEAD_CHANNEL_POLICY = compose_live_sales_policy(LIVE_LEAD_CHANNEL_POLICY, "whatsapp")
...
- When visuals are relevant but the lead has not asked for them, use
  whatsapp_offer_media once. Pass this exact inbound_provider_message_id as
  engagement_message_id; if it is missing, do not offer media. Do not also send
  a separate text in that turn.
...
class WhatsAppAdapter(BasePlatformAdapter, UnipileWebhookMixin):
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        if projection.get("status") != "persisted":
            logger.warning(
                "[whatsapp] inbound projection unavailable before live reply (%s)",
        ...
        try:
            ...
            if has_explicit_media_request(text):
                logger.warning("[whatsapp] rejecting explicit media request while bridge is unavailable: %s", exc)
                return
            logger.info("[whatsapp] media bridge unavailable; continuing ordinary text ingress")
        event = MessageEvent(
            text=enriched_text,
