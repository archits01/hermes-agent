...
try:
    from plugins.platforms._lmi_live_reply_queue import (
        has_explicit_media_request,
        submit_live_reply,
    )
except ImportError:
    ...
    has_explicit_media_request = _queue_module.has_explicit_media_request
...
from plugins.platforms.lmi_unipile_overlay._lmi_media_runtime import (
    MediaOverlayError,
    media_runtime,
    persist_live_inbound_projection,
)
...
# Historical LMI replies were generated under a retired Mon-Sat consultation
# window.  Keep those messages for audit, but do not let them steer a new
# model turn after intake was made 24/7.  Only outbound LMI text is scrubbed;
# a customer's own wording remains evidence exactly as written.
...
LIVE_LEAD_CHANNEL_POLICY = compose_live_sales_policy(LIVE_LEAD_CHANNEL_POLICY, "instagram")
...
- When visuals are relevant but the lead has not asked for them, use
  instagram_offer_media once. Pass this exact inbound_provider_message_id as
  engagement_message_id; if it is missing, do not offer media. Do not also send
  a separate DM in that turn.
...
- For an explicit request for both photos/images and a video/clip, use the
  ...
  corporate: two approved photos plus lmi_video_laser_01) or
  `outdoor_visuals_v1` (outdoor/projection/mapping/facade: two approved photos
  plus lmi_video_projection_01). The single catalog call sends the complete
  photo+video batch. Do not choose a photo-only catalog for a combined request
  and do not substitute a reel/post URL when stored visuals were requested.
- For a photo-only request use projection_highlights_v1 (outdoor/projection)
  or lmi_capabilities_v1 (brochure/capabilities). For a video-only request use
  laser_show_video_v1 or projection_video_v1. Never pass file ids as keys.
- A successful media tool already sends its fixed caption. Never call
  instagram_send_message after a media tool returns sent.
- If a media tool returns review, blocked, or pending, do not claim delivery or
  promise that media will be sent later; give only a truthful hold note.
...
- Every reply ends with either one useful fact plus one question they want to answer, or two concrete half-hour slots from the live availability ledger. Meetings can be requested 24/7; never reject a valid request solely because of the time or day, and never claim fixed consultation hours.
...
class InstagramAdapter(BasePlatformAdapter, UnipileWebhookMixin):
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        projection = await asyncio.to_thread(
            persist_live_inbound_projection, payload
        )
        if projection.get("status") != "persisted":
            logger.warning(
                "[instagram] inbound projection unavailable before live reply (%s)",
                projection.get("reason") or "unknown",
            )
            if has_explicit_media_request(text):
                return
        ...
        try:
            ...
            # Ordinary text must still reach the live reply lane when the optional
            # media bridge is unavailable. Explicit media requests remain fail-closed
            # until the reviewed bridge can bind the exact provider scope.
            if has_explicit_media_request(text):
                logger.warning("[instagram] rejecting explicit media request while bridge is unavailable: %s", exc)
                return
            logger.info("[instagram] media bridge unavailable; continuing ordinary text ingress")
        event = MessageEvent(
            text=enriched_text,
    ...
    async def _tool_send_message(self, args: Dict[str, Any]) -> str:
        ...
        # Instagram post/reel URLs are provider ``media_share`` actions, not
        # the reviewed LMI media catalog.  The generic text tool must never
        # silently turn a model-authored URL into an untracked reel share;
        # require the channel-scoped approved-media tool instead.
        if re.search(r"https?://(?:www\.)?instagram\.com/(?:reel|p|tv)(?:/|$)", text, re.I):
            return json.dumps({
                "status": "review",
                "reason": "instagram_post_urls_require_approved_media_tool",
            })
...
def register(ctx) -> None:
    ...
    _ESCALATION_DB = "/var/lib/lmi-dashboard/unipile_webhooks.db"

    def _escalation_platform(member_id=""):
        """The channel this escalation actually came from, DERIVED - never assumed.

        Was the literal string "instagram", hardcoded. escalate_to_admin is
        registered by this adapter but is agent-global, so it serves every
        channel; all 30 rows claimed instagram, including WhatsApp ones.

        1. leads.platform for this member_id - authoritative.
        2. "@lid" / "@s.whatsapp.net" suffix -> whatsapp. Verified before
           trusting: @lid appears on 7 lead rows, all 7 whatsapp.
        3. None. NOT a guess. "inbound_*" looks usable and is not - it splits
           4 instagram / 4 whatsapp / 3 linkedin.
        """
        import sqlite3 as _sq
        mid = (member_id or "").strip()
        if mid:
            try:
                _c = _sq.connect(_ESCALATION_DB, timeout=10)
                try:
                    row = _c.execute(
                        "SELECT platform FROM leads WHERE member_id=? AND platform IS NOT NULL "
                        "AND platform != '' LIMIT 1", (mid,)).fetchone()
                finally:
                    _c.close()
                if row and row[0]:
                    return str(row[0])
                ...
                pass
            if mid.endswith("@lid") or mid.endswith("@s.whatsapp.net"):
                return "whatsapp"
        return None

    def _escalate(etype, detail, member_id="", chat_id=""):
        """Record an escalation. Returns True only if it reached the admin panel.

        The jsonl write is kept as a second sink but is NOT a fallback - it is a
        file nobody watches, which is exactly how 21 escalations became invisible
        while looking recorded.
        """
        import sqlite3 as _sq, datetime as _dt, json as _j, time as _time
        plat = _escalation_platform(member_id)
        rec = {"ts": _dt.datetime.now(_dt.timezone.utc).isoformat(), "type": etype, "detail": str(detail)[:500],
               "member_id": member_id, "chat_id": chat_id, "platform": plat}
        try:
        ...
        for _attempt in range(4):
            try:
                _cc = _sq.connect(_ESCALATION_DB, timeout=30)
                try:
                    _cc.execute("PRAGMA busy_timeout=30000")
                    _cc.execute("INSERT INTO escalations (created_at, type, detail, member_id, chat_id, platform, status) VALUES (?,?,?,?,?,?, 'open')",
                                (rec["ts"], etype, rec["detail"], member_id, chat_id, plat))
                    _cc.commit()
                finally:
                    _cc.close()
                return True
                ...
                last_err = str(_exc)
                _time.sleep(0.4 * (2 ** _attempt))
        # Loud, not silent. Customer-facing behaviour is deliberately UNCHANGED -
        # a warm holding reply is right either way - but an operator now finds out
        # the panel did not receive this.
        try:
            import sys as _sys
            print(f"[escalation] PANEL WRITE FAILED after 4 attempts, member_id={member_id!r} "
                  f"err={last_err[:200]} - record exists only in escalations.jsonl",
