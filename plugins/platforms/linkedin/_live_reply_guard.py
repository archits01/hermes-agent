...
def _event_bucket(occurred_at: Optional[str]) -> int:
    """Map provider event time to a bounded duplicate-suppression window.

    Providers can emit the same inbound message through webhook and sync paths
    with different timestamps. Exact timestamp fingerprints therefore allowed
    duplicate replies. A five-minute bucket collapses that race while allowing
    an identical message sent later to receive a fresh reply.
    """
    try:
        if occurred_at:
            ...
            return int(stamp.timestamp() // DEDUP_BUCKET_SECONDS)
    except (TypeError, ValueError, OverflowError):
        pass
    return int(time.time() // DEDUP_BUCKET_SECONDS)


def _fingerprint(chat_id: str, sender_id: str, text: str, occurred_at: Optional[str]) -> str:
    payload = "\x1f".join(
        [chat_id.strip(), sender_id.strip(), (text or "").strip(), str(_event_bucket(occurred_at))]
    )
