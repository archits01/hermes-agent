...
class LinkedInAdapter(BasePlatformAdapter):
    ...
    async def _handle_message_event(self, payload: Dict[str, Any]) -> None:
        ...
        if projection.get("status") != "persisted":
            logger.warning(
                "[linkedin] inbound projection unavailable before live reply (%s)",
        ...
        try:
            ...
            if has_explicit_media_request(text):
                logger.warning("[linkedin] rejecting explicit media request while bridge is unavailable: %s", exc)
                return
            logger.info("[linkedin] media bridge unavailable; continuing ordinary text ingress")
        try:
            await submit_live_reply(self, event)
        except Exception:
            logger.exception(
